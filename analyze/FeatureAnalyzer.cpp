...
void Feature::calc(const Region& region, const Mat& frame,void* videoHandler)
{
...
void FeatureAnalyzer::targetUpdate(map<int, Target>& targets,void* videoHandler)
{
    ...
    for (map<int, Target>::iterator it = targets.begin(); it != targets.end(); it++) {
        Target& target = it->second;
        if (target.type != Target::TARGET_LOST) {
            target.feature.calc(target.region, mFrame,videoHandler); //domon..............
        }
    }
}
