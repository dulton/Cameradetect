...
void  CHuman::blobdeal(Mat &displayframe)
{
	...
	//Frame.human.track  = Mat(Frame.variable.frame.rows, Frame.variable.frame.cols, CV_8UC3);//三通道Frame.variable.frame.clone() ; 
	...
	{
		//printf("no objects\n");
		for(int lineNum =0;lineNum < t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.DirectionLines.size();lineNum++)
		{
...
*/
	...
	else//now < pre
	{
		...
		for(int lineNum =0;lineNum<t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.DirectionLines.size();lineNum++)//if(1)//if(Frame.human.humanlist.size()>0)
		{
			for( i=0;i<humanstatis.maxsize[lineNum];i++)//Frame.human.humanlistpre.size() 之前出现的最大id，代表出现的最多人数。out一次，maxsize减1
			{
				flag = 0;
	...
	for(int lineNum =0;lineNum<t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.DirectionLines.size();lineNum++)
	{
		for( i = 0;i<humanlistpro.size();i++)
		{
			...
			humanlistpro[i].pos[lineNum] = pointToline(t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.DirectionLines[lineNum],Point2i(humanlistpro[i].x,humanlistpro[i].y));
	...
	for(unsigned int n=0;n<humanlistpro.size();n++)
	{
		...
		rectangle(displayframe, rt, Scalar( 0, 255, 255 ), 2, 8, 0);//蓝	
	}
...
{	
	...
	{		
		...
		if(rt.x + rt.width < morph.cols && rt.y + rt.height < morph.rows)
		{
			...
			for(unsigned int k=0;k<t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MonitorZoneRects.size() ;k++)//注意，如有多个监控区域，则区域不能相交
			{			
				if((centroidX>t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MonitorZoneRects[k].x)\
					&&(centroidX <t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MonitorZoneRects[k].x +t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MonitorZoneRects[k].width)\
					&&(centroidY>t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MonitorZoneRects[k].y)\
					&&(centroidY <t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MonitorZoneRects[k].y +t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MonitorZoneRects[k].height))
				{	
					label++;
					object.push_back(cv::Point(centroidX,centroidY));
					//humanlist.push_back(blobdata);
					
					rectangle(displayframe, rt, Scalar( 255, 0, 0 ), 2, 8, 0);//蓝
					rectangle(displayframe,cvPoint(centroidX-1,centroidY-1),cvPoint(centroidX+1,centroidY+1),cvScalar(123,123,0),3,8,0); //画出目标的质心
					//alarm = 1;
					char idstr[100]; 
...
void CHuman::algorithmMorphology_Operations( Mat& src, Mat& dst)//将二值图像src中的区域个数统计出来，并去除噪声引起的区域；结果放到dst中
{
	if(0)
	{
		Mat element = getStructuringElement( cv::MORPH_RECT, Size( 12,12));//获得3*3的核  12,12
...
{	
	...
	if((t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.Flag & 0x02)  == 0x02){  
		for(int i=0;i<t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.DirectionLines.size();i++)
		{
			line(displayframe,t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.DirectionLines[i].Start,t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.DirectionLines[i].End,Scalar(255));
		}

	}
	

	if((t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.Flag & 0x01)  == 1){  
	
		for(int ii=0;ii<t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MonitorZoneRects.size();ii++)
		{
			rectangle(displayframe, t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MonitorZoneRects[ii], Scalar( 255, 0, 0 ), 2, 8, 0);//蓝
		}
	}
	...
	if((t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.Flag & 0x02)  == 0x02){  // 出入口只能画一条
	...
	if(humanstatis.numAll >= t_Camera.t_SinCam[m_index].t_Camvarparam.t_CamHumAlarm.MaxNum){
